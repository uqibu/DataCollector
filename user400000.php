...
try {
  for ($i=350001; $i <= 400000; $i++) {
    ...
    $uname1 = trim(str_replace("<span class=\"tit f-ff2 s-fc0 f-thide\">","",$match[0]));
    $uname  = trim(str_replace("</span>","",$uname1));
    ...
    $level = $match[0];
    $level = trim(str_replace("<span class=\"lev u-lev u-icn2 u-icn2-lev\">","",$level));
    $level = trim(str_replace("<i class=\"right u-icn2 u-icn2-levr\"></i></span>","",$level));
    ...
    $province  = isset($whereAll[0])?trim(str_replace("</span>","",$whereAll[0])):"";
    $city      = isset($whereAll[1])?trim(str_replace("</span>","",$whereAll[1])):"";
    /******匹配用户省市end******/
    ...
    if (count($match)<1) {
      ...
      $ageInfo = trim(str_replace("data-age=\"","",$match[0]));
      $ageNum  = trim(str_replace("\"","",$ageInfo));
    ...
    if (count($match)<1) {
      ...
      $summarize = trim(str_replace("个人介绍：","",$match[0]));
      $summarize = trim(str_replace("</div>","",$summarize));
    }
    /******匹配用户简介end******/
